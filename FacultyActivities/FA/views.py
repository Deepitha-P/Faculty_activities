...
def admin_dashboard(request):
    if request.method == 'POST':
        ...
        if category == 'Invited Talks':
            if academic_year_from and academic_year_to != 'None':
                invited_talks = Talks_invited.objects.filter(academic_year__range=[academic_year_from, academic_year_to])
            elif academic_year_from:
                invited_talks = Talks_invited.objects.filter(academic_year=academic_year_from)
            else:
                invited_talks = Talks_invited.objects.all()
...
def fac_dashboard(request):
    if request.method == 'POST':
        ...
        if category == 'Invited Talks':
            invited_talks = Talks_invited.objects.filter(academic_year=yearfrom)
...
def it_detail_enter(request):
    if request.method == 'POST':
        ...
        invited_talks = Talks_invited.objects.filter(academic_year=academic_year_variable)
...
def it_handle_filters(request):
    if request.method == 'POST':
        ...
        invited_talks_queryset = Talks_invited.objects.all()
        if faculty:
...
def it_autosuggest(request):
    ...
    faculty_name = list(Talks_invited.objects.values_list('facultyName', flat=True).distinct())
    event_type = list(Talks_invited.objects.values_list('eventType', flat=True).distinct())
    name_of_event = list(Talks_invited.objects.values_list('nameoftheevent', flat=True).distinct())
    duration = list(Talks_invited.objects.values_list('duration', flat=True).distinct())
    conducted_by = list(Talks_invited.objects.values_list('conductedBy', flat=True).distinct())
    topic_of_talk = list(Talks_invited.objects.values_list('topicOfTalk', flat=True).distinct())
